...
from helper import prepare_data_loader_list
...
napi.download_dataset(filename = f"{configs['Dataset']['name']}/live.parquet", dest_path = f"data/{configs['Dataset']['name']}/live.parquet")
...
maml = MAML(LSTMModel(42,1),lr=1e-2,data_loader_list=prepare_data_loader_list(configs=configs))
...
#     def __init__(self, checkpoint_paths, input_dim=42, output_dim=1):
...
#         # Stack outputs and apply softmax to weights for normalization
#         outputs = torch.stack(outputs, dim=0)  # Shape: [num_models, batch_size]
#         weighted_outputs = torch.softmax(self.weights, dim=0).unsqueeze(1) * outputs.to(self.weights.device)
...
# maml = MAML(EnsembleModel(checkpoint_paths),data_loader_list=prepare_data_loader_list(configs=configs),lr=1e-2)
...
def predict(live_features: pd.DataFrame) -> pd.DataFrame:
    train_loader = maml.find_matching_train_loader(live_features[feature_set].values)
    ...
    for _ in range(5):
        ...
        train_preds = maml(X_train)
        train_loss = loss_fn(train_preds, y_train)
        maml.adapt(train_loss,allow_unused=True)  # Update the cloned model on task's train data
...
# live_predictions = predict(live_features=live_dataset)
...
# live_predictions.to_csv(filename)
