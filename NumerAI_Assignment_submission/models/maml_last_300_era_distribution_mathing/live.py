...
# Load saved model
maml.model.load_state_dict(torch.load(f"saved_models/{configs['Experiment_Name']}/{configs['Model']['name']}.pth"))


# Define your prediction pipeline as a function
def predict(live_features: pd.DataFrame) -> pd.DataFrame:
    live_predictions = maml.model(torch.tensor(live_features[feature_set].values, dtype=torch.float32)).squeeze()
...
with open("maml_last_300_era_distribution.pkl", "wb") as f:
    f.write(p)
