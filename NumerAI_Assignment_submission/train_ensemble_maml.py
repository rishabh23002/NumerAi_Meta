...
Parse.add_argument("--config", type=str, default = 'Assignment1_config_distribution_ensemble.yaml')
...
class EnsembleModel(nn.Module):
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            outputs = [model(x.to(torch.device("cuda"))).to(torch.device("cpu")) for model in self.models]
        ...
        weighted_outputs = torch.softmax(self.weights, dim=0).unsqueeze(1) * outputs
...
maml = MAML(EnsembleModel(checkpoint_paths),lr=1e-2)
